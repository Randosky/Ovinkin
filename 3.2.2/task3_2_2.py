...
def start_processes(vac_name, year, queue):
...
if __name__ == "__main__":
    ...
    for y in years:
        p = Process(target=start_processes, args=(vac, y, q))
        processes.append(p)
        p.start()
    ...
    for p in processes:
        ...
        inp_vacancy_salary.update(dicts_list[2])
        inp_vacancy_count.update(dicts_list[3])
        p.join()
